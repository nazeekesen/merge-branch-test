...
def get_month_labels(now: datetime | None = None):
    """
    Return three x-axis labels for the maturity bar chart based on the current month:
    - PREVIOUS: same month last year
    - CURRENT: current month/year
    - TARGET: same month next year

    Example (if now is Sept 2025):
      ["SEP<br>2024<br>(PREVIOUS)", "SEP<br>2025<br>(CURRENT)", "SEP<br>2026<br>(TARGET)"]
    """
    now = now or datetime.now()
    mon = now.strftime("%b").upper()     # e.g., "SEP"
    return [
        f"{mon}<br>{now.year - 1}<br>(PREVIOUS)",
        f"{mon}<br>{now.year}<br>(CURRENT)",
        f"{mon}<br>{now.year + 1}<br>(TARGET)",
    ]


# --- Figures -----------------------------------------------------------------

def generate_chart(scores, prev_score_missing: bool = False):
    """Generate a bar chart for Platform Maturity scores (Previous / Current / Target)."""
    colors = [
        "#CCCCCC" if prev_score_missing else get_maturity_color(scores[0]),
        get_maturity_color(scores[1]),
        get_maturity_color(scores[2]),
    ]
    display_scores = [
        "N/A" if prev_score_missing else scores[0],
        scores[1],
        scores[2],
    ]
    plotted_scores = [
        0 if prev_score_missing else scores[0],
        scores[1],
        scores[2],
    ]

    # Dynamic labels for all 12 months (based on today's month)
    ...
    fig.update_layout(
        ...
        xaxis=dict(tickangle=0, tickmode="array", tickvals=x_vals, ticktext=labels),
        bargap=0.1,
        plot_bgcolor="white",
        paper_bgcolor="white",
    )
...
def get_current_quarter_and_target():
    ...
    # Current & same-quarter-next-year labels
    current_quarter_label = f"{current_quarter} {year}"
    target_year = year + 1
    target_quarter_label = f"{current_quarter} {target_year}"
...
def generate_single_maturity_chart(rubric_data, width=450):
    """
    Generate a line chart for a single Final Maturity Score rubric (e.g., Overall)
    with lines for Target and optional Decline (if provided on 'Overall').
    """
    ...
    quarters = [current_quarter, target_quarter]
